...
class Framebuffer2DArray : public RenderTarget {
public:

	int width, height, slices;                   //size of FBO
	int fbo;                            //GL identifier
	Texture2D* depthtexture;            //depth texture (z buffer) + stencil
	vector<GLuint> drawbuffers;

	Framebuffer2DArray(int width, int height, int slices, int format) {
		...
		textures.resize(count);
		for (int i = 0; i<count; ++i) {
			textures[i] = new Texture2DArray();
			textures[i]->bind(0);
			glTexImage3D(GL_TEXTURE_2D_ARRAY, 0, format, width, height, slices, 0, GL_RGBA, GL_UNSIGNED_BYTE, NULL);
		...
		for (int i = 0; i<count; ++i) {
			assert(textures[i]->tex != 0);
			glFramebufferTextureLayer(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0 + i, textures[i]->tex, 0, 0);
		}
		glFramebufferTexture(GL_FRAMEBUFFER, GL_DEPTH_STENCIL_ATTACHMENT, depthtexture->tex, 0);
	...
	void bind(int layer) {
		...
		glBindFramebuffer(GL_FRAMEBUFFER, fbo);
		for (unsigned i = 0; i<textures.size(); ++i) {
			glFramebufferTextureLayer(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0 + i, textures[i]->tex, 0, layer);
		}
		glGetIntegerv(GL_VIEWPORT, saved_viewport);
		glViewport(0, 0, width, height);
		glDrawBuffers(drawbuffers.size(), &drawbuffers[0]);
